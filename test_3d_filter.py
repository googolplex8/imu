        ...
        with open(raw_path,'r') as csvfile:
            ...
            for row in plots:
                if(isFirstRow):
                    ...
                    prev_accel = filter(prev_accel, float(row[i]))
                    prev_accel = prev_accel + (calculate_gravity(float(row[11]), float(row[10])))[i]
                    output[i].append(str(prev_accel))
            #counter+=1
        
    print(str(len(output[2])))
